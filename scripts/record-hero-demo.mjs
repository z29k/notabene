// Record the hero demo GIF used in the READMEs (assets/notabene-demo.gif).
//
// Self-contained and deterministic — mirrors scripts/record-mobile-demo.mjs:
//   1. regenerate a clean .demo fixture WITH --git (so /review shows real diffs),
//   2. spawn `notabene dev` against it on its own port,
//   3. drive desktop Chrome (Playwright): select a passage → leave a comment, then go to
//      /review and approve the real git diff the agent proposed,
...
// Usage:  node scripts/record-hero-demo.mjs   (or: npm run gen:hero-gif)
...
const OUT_GIF = path.join(REPO, "assets/notabene-demo.gif");
const PORT = 3829;
const VIEWPORT = { width: 1280, height: 820 };

// The passage a reviewer selects, and the comment they leave.
...
async function record(base, videoDir) {
  ...
  // beat 1 — land on a doc page
  await page.goto(`${base}/docs`, { waitUntil: "networkidle" });
  await addChrome();
  await wait(1100);

  // beat 2 — select a passage → the floating Comment button appears
  ...
  await moveCursor("#cmt-float");
  await wait(400);

  // beat 3 — click Comment → the compose popover, type, save
  await page.locator("#cmt-float").click();
  ...
  await wait(1500); // the passage becomes a persistent highlight

  // beat 4 — the agent has already proposed edits (fixture, approve mode): open /review
  await page.goto(`${base}/review`, { waitUntil: "networkidle" });
  await page.waitForSelector(".rev-card", { timeout: 10_000 });
  await page.waitForSelector(".rev-card .nb-diff, .rev-card .diff, .rev-card table", { timeout: 8000 }).catch(() => {});
  await wait(1900); // read the real git diff

  // beat 5 — approve the diff
  await moveCursor('.rev-card button[data-act="approve"]');
  await wait(400);
  await page.locator('.rev-card button[data-act="approve"]').first().click();
  await wait(2000);
...
function toGif(webm) {
  // The unified diff (coloured code) needs a slightly richer palette than the flat UI.
  const vf =
